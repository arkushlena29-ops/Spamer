...
import {
	runDispatch,
	type RunConfig,
} from "../../scripts/email-dispatch/runner";
...
class WorkerManager {
	...
	public resetStatus(initialState: Partial<WorkerStatus> = {}): void {
		const persisted = loadState();
		this.status = {
			running: false,
			stopping: false,
			...
			error: null,
			totalRows: initialState.totalRows ?? 0,
			lastProcessedIndex: initialState.lastProcessedIndex ?? -1,
			totalSent: initialState.totalSent ?? 0,
