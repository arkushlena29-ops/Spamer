...
const PLACEHOLDERS: { key: string; label: string }[] = [
	{ key: "ПІБ", label: "{{ПІБ}}" },
	{ key: "Номер договору", label: "{{Номер договору}}" },
	{ key: "Email", label: "{{Email}}" },
	{ key: "Залишок заборгованості", label: "{{Залишок заборгованості}}" },
	{ key: "Первинний кредитор", label: "{{Первинний кредитор}}" },
	{ key: "Тіло", label: "{{Тіло}}" },
	{ key: "Відсотки", label: "{{Відсотки}}" },
	{ key: "Комісії", label: "{{Комісії}}" },
	{ key: "Штрафи", label: "{{Штрафи}}" },
	{ key: "Борг за відступленням", label: "{{Борг за відступленням}}" },
	{ key: "Етап", label: "{{Етап}}" },
	{ key: "Статус", label: "{{Статус}}" },
	{ key: "Статус Email", label: "{{Статус Email}}" },
	{ key: "Назва пула", label: "{{Назва пула}}" },
];
...
export default function TemplatesTab() {
	...
	const loadSampleRow = async () => {
		...
		try {
			...
			console.error("Failed to load sample row", e);
		} finally {
	...
	// ── Preview rendering ────────────────────────────────────────────────────────
	const nameParts = (previewData["ПІБ"] || "").split(" ");
	const previewVars: Record<string, string> = {
		...previewData,
		name: previewData["ПІБ"] || "Уважаемый клиент",
		firstName: nameParts[0] || "",
		lastName: nameParts.slice(1).join(" ") || "",
		to: previewData["Email"] || "",
		subject: draft
			? substitute(draft.subject, {
					...previewData,
					name: previewData["ПІБ"] || "Уважаемый клиент",
					firstName: nameParts[0] || "",
					lastName: nameParts.slice(1).join(" ") || "",
	...
	const previewKey = JSON.stringify({ previewVars, draft: draft?.id });

	const isDirty =
		draft && templates[selected]
			...
			<div
					...
					<button
						...
						<div
							style={{
								fontSize: "11px",
								fontWeight: 700,
								color: selected === idx ? "#93c5fd" : dimText,
								marginBottom: "3px",
							}}>
							Шаблон {tpl.id}
						</div>
						<div
							style={{
								fontSize: "12px",
								color: selected === idx ? bodyText : muted,
								lineHeight: 1.3,
							}}>
							{tpl.name}
						</div>
						<div
							style={{ marginTop: "5px", fontSize: "10px", color: "#334155" }}>
				...
				<div
							...
							return (
								<button
									...
									style={{
										textAlign: "left",
										background: "#0f172a",
										border: `1px solid ${value ? "#059669" : "#1e293b"}`,
										borderRadius: "4px",
										padding: "3px 7px",
										fontSize: "11px",
										color: value ? "#86efac" : "#7dd3fc",
										cursor: "pointer",
										fontFamily: '"Geist Mono", Consolas, monospace',
									}}>
									<div>{label}</div>
									{value && (
										<div style={{ fontSize: "10px", color: "#94a3b8", marginTop: "1px", maxWidth: "150px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
											{value}
										</div>
									)}
								</button>
					...
					<div style={{ marginTop: "6px", fontSize: "10px", color: "#334155" }}>
						{previewData["ПІБ"] ? "Данные загружены" : "Нажмите 'Загрузить из XLSX'"}
					</div>
				</div>
			</div>
					...
					<div style={{ flex: 1, display: "flex", gap: "14px", minHeight: 0 }}>
						...
						<div
							...
							<div
								...
								<button
									onClick={loadSampleRow}
									disabled={loadingSample}
									style={{
										...btnStyle("#059669", "#fff", loadingSample),
										padding: "5px 12px",
									}}>
									{loadingSample ? "Загрузка..." : "Загрузить из XLSX"}
								</button>
							</div>
							...
							<div
								...
								<span style={{ fontSize: "13px", color: bodyText }}>
									{previewVars.subject}
								</span>
							</div>
						</div>
