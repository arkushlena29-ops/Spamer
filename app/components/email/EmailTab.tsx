			...
			<section>
				<div
					...
					<span style={{ fontSize: "13px", fontWeight: 600, color: bodyText }}>
						SMTP аккаунты{" "}
						<span style={{ color: dimText, fontWeight: 400 }}>
							({accounts.length})
						</span>
					</span>
					<button onClick={openAdd} style={btn("#1e3a5f", "#93c5fd")}>
						+ Добавить аккаунт
					</button>
				</div>
				...
				{accounts.length === 0 ? (
					...
					<div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
						{accounts.map((acc, i) => {
							...
							const barColor =
								pct >= 100 ? "#ef4444" : pct >= 90 ? "#f59e0b" : "#22c55e";
							...
							return (
								<div
									...
									style={{
										background: surface,
										border: `1px solid ${exhausted ? "#7f1d1d" : border}`,
										borderRadius: "8px",
										padding: "12px 16px",
									}}>
									<div
										...
										<div style={{ flex: 1, minWidth: 0 }}>
											<div
												style={{
													fontSize: "13px",
													fontWeight: 600,
													color: exhausted ? "#fca5a5" : bodyText,
													overflow: "hidden",
											...
											<div
												...
												{acc.host}:{acc.port} ·{" "}
												{acc.secure ? "TLS" : "STARTTLS"}
											</div>
										</div>
										<div style={{ display: "flex", gap: "8px" }}>
											<button
												onClick={() => openEdit(acc)}
												style={btn("#1e293b", "#94a3b8")}>
												Изменить
											</button>
											<button
												onClick={() => handleDelete(acc.id)}
												style={btn("#450a0a", "#fca5a5")}>
												Удалить
											</button>
										</div>
									</div>
						...
						<div
							...
							<div>
								...
								<input
									...
									value={form.dailyLimit ?? 500}
									onChange={(e) =>
										setForm({
											...form,
											dailyLimit: Math.max(1, Number(e.target.value)),
										})
									}
								/>
							</div>
	...
	const [selectedStatuses, setSelectedStatuses] = useState<Set<string>>(() => {
		...
		if (saved) {
			try {
				...
				// Check if any of the saved filter IDs are valid (exist in STATUS_OPTIONS)
				const validIds = parsed.filter((id: string) =>
					STATUS_OPTIONS.some((opt) => opt.id === id),
				);
				if (validIds.length > 0) {
					return new Set(validIds);
				} else {
					// All saved filters are invalid, clear localStorage and use defaults
					localStorage.removeItem("emailStatusFilters");
					console.log(
						"Resetting default status filters - old filter IDs were invalid",
					);
				}
			} catch {}
		}
		...
		<div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
			...
			<div
				...
				<label
					...
					/>
					<span
						style={{ fontSize: "13px", color: "#94a3b8", userSelect: "none" }}>
						Тестовый режим
					</span>
				</label>
				...
				<div style={{ marginLeft: "auto", display: "flex", gap: "10px" }}>
					<button
						...
						style={{
							...btn("#1e293b", "#94a3b8", isActive || resetting),
							padding: "8px 16px",
							fontSize: "13px",
							fontWeight: 600,
						}}>
					...
					<button
						...
						style={{
							...btn("#2563eb", "#fff", isActive || starting),
							padding: "10px 22px",
							fontSize: "14px",
							fontWeight: 700,
						}}>
					...
					<button
						onClick={handleStop}
						disabled={!isActive || status.stopping}
						style={{
							...btn("#7f1d1d", "#fca5a5", !isActive || status.stopping),
							padding: "10px 22px",
							fontSize: "14px",
							fontWeight: 700,
						}}>
				...
				<div
					...
					<div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
						...
						<span
							style={{
								fontSize: "11px",
								color: sseConnected ? "#6ee7b7" : "#ef4444",
							}}>
				...
				<div
					...
					{logs.length === 0 ? (
						<span style={{ color: "#334155" }}>
							Логов пока нет — нажмите «Начать отправку».
						</span>
					) : (
						logs.map((e, i) => (
							<div
...
// ─────────────────────────────────────────────────────────────────────────────
// TableEditTab — filters for table editing
// ─────────────────────────────────────────────────────────────────────────────

function TableEditTab() {
	const [poolNameFilter, setPoolNameFilter] = useState<string>(() => {
		const saved = localStorage.getItem("emailPoolNameFilter");
		return saved !== null ? saved : "";
	});
		...
		try {
			await fetch("/api/email/generate", {
				...
				body: JSON.stringify({
					poolNameFilter,
					creditorFilter,
					statusFilters: Array.from(selectedStatuses),
				}),
			});
		} finally {
			setGenerating(false);
...
function Stat({
	...
	return (
		<div>
			...
			<div
				style={{ fontSize: "15px", fontWeight: 700, color: color ?? bodyText }}>
				{value}
			</div>
		</div>
	);
...
type SubTab = "dispatch" | "accounts" | "templates" | "table";

const SUB_TABS: { id: SubTab; label: string }[] = [
	{ id: "dispatch", label: "Рассылка" },
	{ id: "accounts", label: "Аккаунты" },
	{ id: "templates", label: "Шаблоны" },
	{ id: "table", label: "Изменить таблицу" },
];
				...
				{SUB_TABS.map(({ id, label: lbl }) => (
					<button
						...
						style={{
							...
							border: "none",
							borderBottom:
								sub === id ? "2px solid #3b82f6" : "2px solid transparent",
