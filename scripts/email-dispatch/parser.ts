...
const EXCEL_PATH = resolve(process.cwd(), "public", "emails.xls");
...
function normalizeRow(raw: RawRow, rowIndex: number): EmailRow | null {
  ...
  return {
    rowIndex,
    email,
    firstName: pick(raw, "First Name", "FirstName", "first_name", "Имя", "Ім'я", "Name"),
    lastName: pick(raw, "Last Name", "LastName", "last_name", "Фамилия", "Прізвище"),
...
/**
 * Parse public/emails.xls and return a deduplicated, validated array of rows.
 * Called once at startup — subsequent access is from the in-memory array.
 */
export function parseEmailsFile(log: Logger = (e) => console.log(`[Parser] ${e.message}`)): EmailRow[] {
  ...
  log({ level: "system", message: `Sheet "${sheetName}" — ${rawRows.length} raw rows` });
  ...
  log({ level: "system", message: `${rows.length} valid unique rows after filtering` });
